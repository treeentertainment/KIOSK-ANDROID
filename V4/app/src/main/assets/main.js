...
window.addEventListener('message', (event) => {
  ...
  } else if(event.data.type === 'newOrder') {
    displayorders(event.data.number);
  } else if(event.data.type === 'noselect' ) {
  if (orderwindow && !orderwindow.closed) {
    orderwindow.close();
    orderwindow = null;
  }
  } else if(event.data.type === 'original' ) {
...
function selectoption(event, item) {
  ...
  // AndroidBridge에만 전달 (Android에서 subWebView 로딩 후 전달함)
  if (window.AndroidBridge && typeof window.AndroidBridge.postMessage === 'function') {
    window.AndroidBridge.postMessage(JSON.stringify(item));
  } else {
    console.error("AndroidBridge is not defined");
  }
}
...
function checkAuthState() {
  ...
  function loginfinish() {
    window.AndroidBridge.requestMenuData(String(number));
  }
