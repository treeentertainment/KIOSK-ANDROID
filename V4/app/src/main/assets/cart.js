...
function addItemToOrder({ id, image, name, price, quantity, options, max }) {
  ...
  let order = getOrder(); // Get the latest order
    ...
    if(existingIndex !== -1) {
        const message = JSON.stringify({ type: "UpdateOrder", id: id, quantity: order[existingIndex].quantity });
        window.AndroidBridge.postMessage(message);
    } else {
      const message = JSON.stringify({ type: "newOrder", number: order.length });
      window.AndroidBridge.postMessage(message);
    }
}
